...
for execucao in range(n_execucoes):
    for tamanho in tamanhos_esperados:
        ...
        indice += 1
        linha3 = list(map(float, linhas[indice].strip().split()))
        indice += 1

        if int(linha1[0]) != tamanho or int(linha2[0]) != tamanho or int(linha3[0]) != tamanho:
            print(f"Tamanho inesperado (esperado: {tamanho}, obtido: {linha1[0]}, {linha2[0]} e {linha3[0]})")
            continue

        data1.setdefault(tamanho, []).append(linha1[1])
        data2.setdefault(tamanho, []).append(linha2[1])
        data3.setdefault(tamanho, []).append(linha3[1])
...
tempos3_med = [np.mean(data3[t]) for t in tamanhos]
...
# Regressão linear para Medição 3
modelo3 = LinearRegression()
modelo3.fit(tamanhos_np, tempos3_med)
y3_pred = modelo3.predict(tamanhos_np)
r2_3 = r2_score(tempos3_med, y3_pred)
...
# Gráfico para a terceira medição (ajuste na janela de visualização)
plt.subplot(1, 3, 3)
plt.plot(tamanhos, tempos3_med, marker='o', color='purple', label='Média dos tempos')
plt.plot(tamanhos, y3_pred, color='red', linestyle='--', label=f'Regressão Linear (R² = {r2_3:.4f})')
plt.xlabel("Tamanho da entrada")
plt.ylabel("Tempo de execução (s)")
plt.title("Medição 3")
plt.ylim(min(tempos3_med) * 0.8, max(tempos3_med) * 1.2)
plt.legend()
plt.grid(True)
